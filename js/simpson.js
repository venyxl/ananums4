...
// Conversion np.xxx → sympy pour le calcul exact
function toSympy(expr) {
  return expr
    ...
    .replace(/np\.abs/g, 'Abs').replace(/np\.pi/g, 'pi').replace(/np\.e\b/g, 'E')
    .replace(/np\./g, '');
}
...
async function calculate() {
  ...
  if (m < 1) { alert('⚠️ m doit être un entier positif.'); return; }
...
def simpson(f, a, b, m):
    ...
    for i in range(1, 2 * m):
        ...
        s += coeff * f(xi)
        steps.append({'i': i, 'xi': round(xi, 6), 'fxi': round(f(xi), 6), 'coeff': coeff, 'contrib': round(coeff * f(xi), 6)})
    I = (h / 3) * (f(a) + f(b) + s)
...
function showResults(data, a, b, m) {
  ...
  document.getElementById('result-info').textContent  = `h = ${data.h} — ${2*m} sous-intervalles — ${2*m+1} points`;
...
function fillTable(steps, fa, fb) {
  ...
  trA.innerHTML = `<td>0</td><td><strong>${document.getElementById('val-a').value}</strong></td><td>${fa}</td><td style="color:var(--accent);font-weight:700;">1</td><td>${fa}</td>`;
  tbody.appendChild(trA);
  steps.forEach((row, i) => {
    const tr = document.createElement('tr');
    tr.style.animation = `fadeIn 0.3s ease ${i*0.03}s both`;
    tr.innerHTML = `<td>${row.i}</td><td><strong>${row.xi}</strong></td><td>${row.fxi}</td>
      <td style="color:${row.coeff===4?'rgba(79,70,229,1)':'rgba(16,185,129,1)'};font-weight:700;">${row.coeff}</td><td>${row.contrib}</td>`;
    tbody.appendChild(tr);
  });
  const trB = document.createElement('tr');
  trB.innerHTML = `<td>${steps.length+1}</td><td><strong>${document.getElementById('val-b').value}</strong></td><td>${fb}</td><td style="color:var(--accent);font-weight:700;">1</td><td>${fb}</td>`;
...
function drawChart(data) {
  ...
  chart = new Chart(ctx, {
    ...
    data: {
      labels: data.x_plot,
      datasets: [
        { label: 'f(x)', data: data.y_plot, borderColor: 'rgba(79,70,229,1)', backgroundColor: 'rgba(79,70,229,0.15)', borderWidth: 2.5, pointRadius: 0, fill: true, tension: 0.4 },
        { label: 'Points de subdivision', data: data.x_nodes.map((x,i)=>({x,y:data.y_nodes[i]})), borderColor: 'rgba(239,68,68,1)', backgroundColor: 'rgba(239,68,68,1)', pointRadius: 5, showLine: false, type: 'scatter' }
      ]
    },
    options: {
      responsive: true,
